...
//    let projectionMatrix = simd_float4x4(orthographicProjectionWithLeft: 0, top: 1.08 , right: 1.92, bottom: 0 , near: -1, far: 1)
    ...
    func makeResources() {
        ...
        vertexDescriptor.attributes[6].format = .float; // rotation
        vertexDescriptor.attributes[6].offset = MemoryLayout<Float>.stride * 8
        vertexDescriptor.attributes[6].bufferIndex = 1;

        vertexDescriptor.attributes[7].format = .float2 // velocity
        vertexDescriptor.attributes[7].offset = MemoryLayout<Float>.stride * 9
    ...
    func draw(_ renderCommandEncoder: MTLRenderCommandEncoder) {
        ...
        renderCommandEncoder.drawPrimitives(type: .triangleStrip, vertexStart: 0, vertexCount: 3, instanceCount: particleCount)
    }
}
